...
for layer in $(pwd)/data/image/*/layer.tar; do
    ...
    if [ -n "$(tar -tf ${layer} | grep 'opt/gradle')" ]; then echo found opt/gradle in ${layer}; LAYERS+=(${layer}); fi
done
...
tar_directories=($(tar tf data/layer.tar | grep -E '.*/$' | grep -v 'gradle' | sort -r -n))
